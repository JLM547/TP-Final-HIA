...
Las medidas de seguridad solicitadas en el enunciado se encuentran implementadas y documentadas en `docs/seguridad.md`. Resumen de uso:

- **Certificados HTTPS**: generar certificados autofirmados para entornos locales con  
  `./scripts/generate-selfsigned-cert.sh ssl localhost` (Linux/macOS) o  
  `.\scripts\generate-selfsigned-cert.ps1 -OutputDir ssl -DnsName localhost` (Windows, requiere Python 3 y el paquete `cryptography`).
- **Firewall**: aplicar reglas con  
  `sudo ./scripts/setup-firewall.sh` (Linux) o  
  `.\scripts\setup-firewall.ps1 -Reset` (Windows).  
  Verifica el estado con `ufw status verbose` / `Get-NetFirewallRule`.
- **Mitigación DDoS**: Nginx aplica rate limiting (10 rps, burst 20) y el firewall agrega límites de conexiones simultáneas. Puedes validar con `ab -n 1000 -c 100 https://localhost/`.
- **Accesos protegidos**: todas las credenciales sensibles se cargan desde `.env` (no versionado). Asegúrate de definir `MONGO_ROOT_PASSWORD`, `ME_BASICAUTH_PASSWORD`, `JWT_SECRET`, `GRAFANA_PASSWORD`, etc. antes de levantar los contenedores.

> Para más detalles, evidencia y próximos pasos sugeridos, revisar `docs/seguridad.md`.
