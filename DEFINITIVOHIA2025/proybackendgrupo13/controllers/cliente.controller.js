...
exports.getClientes = async (req, res) => {
  ...
  try {
    // Paginación para evitar cargar demasiados clientes de una vez
    ...
    let totalClientes;
    try {
      totalClientes = await Cliente.countDocuments(query);
    } catch (countError) {
      console.error('Error al contar clientes:', countError);
      totalClientes = 0;
    }

    const clientes = await Cliente.find(query).populate({
      path: 'usuarioId',
    ...
    console.log('ClienteController: Total de clientes (sin límite):', totalClientes);
    ...
    res.status(200).json({
      ...
      paginacion: {
        ...
        tieneSiguiente: page < Math.ceil(totalClientes / limit),
