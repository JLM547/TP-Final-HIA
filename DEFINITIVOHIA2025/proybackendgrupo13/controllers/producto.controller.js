...
exports.getProducts = async (req, res) => {
  try {
    // Paginación para evitar cargar demasiados productos de una vez
    const page = parseInt(req.query.page) || 1;
    const limit = Math.min(parseInt(req.query.limit) || 100, 500); // Productos pueden tener límite más alto
    ...
    let totalProductos;
    try {
      totalProductos = await Producto.countDocuments(query);
    } catch (countError) {
    ...
    res.status(200).json({
      ...
      paginacion: {
        ...
        tieneSiguiente: page < Math.ceil(totalProductos / limit),
