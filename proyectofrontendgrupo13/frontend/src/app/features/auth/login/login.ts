...
import { CommonModule, isPlatformBrowser } from '@angular/common'; // Necesario para directivas como ngIf, ngFor y verificación de plataforma
...
export class LoginComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  initializeGoogleIdentityServices(): void {
    ...
    if (!isPlatformBrowser(this.platformId)) {
      console.warn('Google Identity Services no se puede inicializar en el servidor');
      return;
    }
  ...
  private setupGoogleIdentityServices(): void {
    // Verificar que estamos en el navegador (no en SSR)
    if (!isPlatformBrowser(this.platformId)) {
      console.warn('Google Identity Services no se puede configurar en el servidor');
      return;
    }
  ...
  private loadGoogleIdentityServices(): Promise<void> {
    return new Promise((resolve, reject) => {
      // Verificar que estamos en el navegador (no en SSR)
      if (!isPlatformBrowser(this.platformId)) {
        reject(new Error('Google Identity Services solo puede cargarse en el navegador'));
        return;
      }
