...
exports.getUsuarios = async (req, res) => {
    try {
        // Paginación para evitar cargar demasiados usuarios de una vez
        ...
        // Filtrar por rolId si se proporciona como query param
        if (req.query.rolId) {
            query.rolId = req.query.rolId;
        }

        // Contar total
        let totalUsuarios;
        try {
            totalUsuarios = await Usuario.countDocuments(query);
        } catch (countError) {
            console.error('Error al contar usuarios:', countError);
            totalUsuarios = 0;
        }

        // Usamos la función auxiliar para poblar los perfiles y agregamos paginación
        const usuarios = await populateUserProfiles(Usuario.find(query).select('-password'))
            .limit(limit)
            .skip(skip)
            .lean();
        ...
        res.status(200).json({
            ...
            paginacion: {
                ...
                tieneSiguiente: page < Math.ceil(totalUsuarios / limit),
